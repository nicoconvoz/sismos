...
- **SMN Argentina** (`lib/smn.js`): official weather warnings (zonda wind, snowfall, heat/cold waves, hail, storms) as WMO CAP XML from `ssl.smn.gob.ar/CAP/AR.php` — the JSON API sits behind a Cloudflare challenge, the CAP index does not, but it content-negotiates unpredictably (HTML href= vs RSS <link>), so the parser matches the .xml URL shape itself. Each alert carries a real polygon (centroid = location), the forecaster's description (`details`: gusts, cm of snow, temperatures) and the official zone in the FILE NAME (`area`: Cordillera/Llanura/Patagonia — `<areaDesc>` ships empty). CAP severity maps Minor/Moderate/Severe/Extreme → green/yellow/orange/red (yellow is its own band in `alertMagnitude`).
