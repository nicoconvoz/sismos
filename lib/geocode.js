...
/**
 * 8-wind sector index FROM the city TO the event location, clockwise from
 * north (0 = N, 1 = NE, … 7 = NW). Language-neutral: clients map it to their
 * own compass letters.
 */
export function compassIndex(cityLat, cityLon, lat, lon) {
...
/**
 * 8-wind Spanish compass direction FROM the city TO the event location
 * ("al NE de X" = the event lies NE of city X).
 */
export function compass8(cityLat, cityLon, lat, lon) {
  return COMPASS_ES[compassIndex(cityLat, cityLon, lat, lon)];
}
...
/**
 * Structured twin of reverseGeocode for client-side localization: the raw
 * pieces ({ name, admin1, cc, distKm, dir }) so the frontend can phrase the
 * label in the viewer's language. Same ocean cutoff as the label path.
 */
export function reverseGeocodeData(lat, lon, cities = loadCities()) {
  const city = nearestCity(lat, lon, cities);
  if (!city || city.distKm > OCEAN_CUTOFF_KM) return null;
  return {
    name: city.name,
    ...
    cc: city.cc,
    distKm: city.distKm,
    dir: compassIndex(city.lat, city.lon, lat, lon)
  };
}

/**
 * Annotate each event with `near`: the offline nearest-city label, or null
 * for mid-ocean locations. Mutates and returns the same array; runs once per
 * API cache rebuild so tooltips and cards can render the label synchronously.
 */
export function annotateNear(events) {
  for (const q of events) {
    const data = reverseGeocodeData(q.lat, q.lon);
    q.nearData = data;
    q.near = data
      ? buildNearLabel(
          { name: data.name, admin1: data.admin1, cc: data.cc },
          data.distKm,
          COMPASS_ES[data.dir]
        )
      : null;
