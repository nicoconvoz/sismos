...
// GDACS alert levels carry an expert-computed humanitarian impact estimate;
// each level anchors a tier band, with alertscore refining position inside it.
// The +score contribution is capped so a level can never leak into the band
// above (green stays "mediano", orange "grande", red "gigante").
const ALERT_BASE = { green: 3.2, orange: 5.2, red: 7.2 };
const ALERT_SCORE_STEP = 0.4;
const ALERT_SCORE_MAX = 4;

/** Magnitude for an alert-graded event (GDACS Green/Orange/Red + alertscore). */
export function alertMagnitude(alertlevel, alertscore = 0) {
  const base = ALERT_BASE[String(alertlevel || '').toLowerCase()];
  if (base === undefined) return 2.5;
  const score = Math.min(ALERT_SCORE_MAX, Math.max(0, Number(alertscore) || 0));
  return clampMagnitude(base + score * ALERT_SCORE_STEP);
}
