...
/**
 * Ordered search terms to try until coverage is found. The press mostly
 * covers events at STATE level ("sismo Oaxaca"), not the nearest village the
 * geocoder names — so: city (local press only) -> admin1 -> country. Named
 * storms search strictly by name: a country fallback would mix in every
 * other storm of the season.
 */
export function termLadder({ localPress, place, admin1, country, properName = null }) {
...
/**
 * Keep only items published STRICTLY after the event started, so coverage
 * of an earlier similar event in the same place never mixes in. Invalid or
 * missing `since` filters nothing — better too much news than a silently
 * empty list.
 */
export function filterBySince(items, sinceIso, marginMs = 0) {
